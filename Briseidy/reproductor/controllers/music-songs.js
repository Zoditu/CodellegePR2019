...
class MusicSongs extends EndpointBase
{
    ...
    async GETBySearch(search)
    {
        ...
        if(search) //http://localhost:667/music/songs?search=michael+jackson
        {
                    ...
                    if(songs.length > 0 )
                    {
                        lastIndex = songs[songs.length - 1].id; //numero de canciones encontradas menos uno y me da el id del ultimo 
                    }
                    this.result.data = 
                        {
                    ...
                    else
                    {
                        ...
                        {
                            message:'Insufficient search criteria.',
                            warning:'"search" parameter is required to perfom this query',
                            result: 'WARNING'
                        };
                        this.status = 406;
                        this.FinishRequest();
