/*class EndpointBase
{
   ...
   CheckHeaders ( ...headers) //initialid, max
   {
      ...
      for(var i = 0; i < headers.length; i++)
      {
         ...
         if(!_headers[header])//nombre de las propiedades que quiero revisar
         {

         this.result.data =
         {
            message: 'Invalid endpoint request.',
            error: 'Invalid headers on sent request. initialID or max not defined',
            result: 'ERROR'
         }
         this.status = 412
         this.FinishRequest();
   ...
   //CheckHeaders( 'initialid', 'max', 'x-owner' ) RWAP
  ...
  InvalidMethod(METHOD)//se coloca la palabra static 
   {
      if (METHOD === 'OPTIONS')
      {
         var headers = {};
   ...
   ToRegex(text)
{
   ...
   EnableCORSForLocal()
   {
      ...
      this.response.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS, PUT, PATH, DELETE');
      this.response.setHeader('Access-Control-Allow-Headers', 'X-Requested-with,contetn-type');
